...
renderCUDA(
	...
	const float* __restrict__ dL_dsemanticpixels,
...
)
{
	...
	if (inside)
	{
		...
		for (int i = 0; i < S; i++)
			dL_dsemanticpixel[i] = dL_dsemanticpixels[i * H * W + pix_id];
		if(dL_invdepths)
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			const float dchannel_dsemantics = alpha * T;
			...
			for (int ch = 0; ch < S; ch++)
			{
				...
				const float dL_dsemanticchannel = dL_dsemanticpixel[ch];
				/**************************************************************************************************/
				// dL_dalpha += (f - accum_semantics_rec[ch]) * dL_dsemanticchannel; // Do not add to alpha gradient.
				/**************************************************************************************************/
				// Update the gradients w.r.t. semantic feature of the Gaussian. 
				// Atomic, since this pixel is just one of potentially
				// many that were affected by this Gaussian.
				atomicAdd(&(dL_dsemantics[global_id * S + ch]), dchannel_dsemantics * dL_dsemanticchannel); 
			}
...
void BACKWARD::render(
	...
	const float* dL_dsemanticpixels,
